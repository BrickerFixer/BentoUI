document.addEventListener('DOMContentLoaded', function() {
  ...
  // Plugin toggles
  const pluginToggles = document.querySelectorAll('.plugin-list input[type="checkbox"]');
  pluginToggles.forEach(toggle => {
    toggle.addEventListener('change', function() {
      const blockType = this.value;
      const blocks = document.querySelectorAll(`.bento-block[data-type="${blockType}"]`);
    ...
    // Initialize checkboxes based on visible blocks
    const blockType = toggle.value;
    const blocks = document.querySelectorAll(`.bento-block[data-type="${blockType}"]`);
    toggle.checked = blocks.length > 0 && getComputedStyle(blocks[0]).display !== 'none';
  });
